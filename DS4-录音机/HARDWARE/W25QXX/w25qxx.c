...
//写入两个字节的数据
//block   块号    每个块64K    一个音频分24个块    共1536K   1.5M
//因此   第一个音频的首地址是0x000000   首块是0x00
//			第二个音频首地址 0x180000            0x18
//                       0x300000            0x30
//											 0x480000						 0x48
extern u32 wcount,rcount;
void Write_2byte(u32 block,u8 writechar[2]){

	
	SPI2_ReadWriteByte(W25X_PageProgram);      	//发送写页命令   
  SPI2_ReadWriteByte((u8)((block)>>16)); 	//发送24bit地址    
  SPI2_ReadWriteByte((u8)((block)>>8));   
  SPI2_ReadWriteByte((u8)block);
	SPI2_ReadWriteByte(writechar[0]);
	SPI2_ReadWriteByte(writechar[1]);
}
